var dp = {
    sh : {
        Toolbar : {}, Utils : {}, RegexLib : {}, Brushes : {}, Strings : {}, Version : "1.4.1"
    }
};
dp.sh.Strings = 
{
...
dp.sh.Toolbar.Create = function (_11)
{
    ...
    div.className = "tools";
    div.innerHTML = '<b style="color:#000;">Code</b>&nbsp;&nbsp;&nbsp;';
...
dp.sh.RegexLib = 
{
    MultiLineCComments : new RegExp("/\\*[\\s\\S]*?\\*/", "gm"), SingleLineCComments : new RegExp("//.*$", 
    "gm"), SingleLinePerlComments : new RegExp("#.*$", "gm"), DoubleQuotedString : new RegExp("\"(?:\\.|(\\\\\\\")|[^\\\"\"])*\"", 
    "g"), SingleQuotedString : new RegExp("'(?:\\.|(\\\\\\'))*'", "g")
};
...
dp.sh.Highlighter.prototype.IsInside = function (_2d)
{
    ...
    for (var i = 0; i < this.matches.length; i++)
    {
        ...
        if ((_2d.index > c.index) && (_2d.index < c.index + c.length)) {
            return true;
        }
    }
    return false;
...
dp.sh.Highlighter.prototype.Highlight = function (_4b)
{
    ...
    function Unindent(str)
    {
        ...
        {
            if (Trim(_4f[i]).length == 0) {
                continue;
            }
            var _54 = _51.exec(_4f[i]);
            if (_54 != null && _54.length > 0) {
                min = Math.min(_54[0].length, min);
            }
        }
        if (min > 0) {
    ...
    for (var i = 0; i < this.matches.length; i++)
    {
        ...
        if (_5c == null || _5c.length == 0) {
            continue;
        }
        this.AddBit(Copy(this.code, pos, _5c.index), null);
        this.AddBit(_5c.value, _5c.css);
        pos = _5c.index + _5c.length;
    }
...
dp.sh.Brushes.CSS = function ()
{
    ...
    {
        regex : new RegExp('\\#[a-zA-Z0-9]{3,6}', 'g'), css : 'colors'
    },
    {
        regex : new RegExp('(\\d+)(px|pt|\:)', 'g'), css : 'string'
    },
    {
        regex : new RegExp(this.GetKeywords(keywords), 'gm'), css : 'keyword'
    },
    ...
    {
        regex : new RegExp(this.GetKeywords(fonts), 'g'), css : 'string'
    }];
...
{
    var keywords = 'and assert break class continue def del elif else except exec ' + 'finally for from global if import in is lambda not or object pass print ' + 'raise return try yield while';
    ...
    {
        regex : new RegExp(this.GetKeywords(builtins), 'gm'), css : 'builtins'
    },
    {
        regex : new RegExp(this.GetKeywords(magicmethods), 'gm'), css : 'magicmethods'
    },
    {
        regex : new RegExp(this.GetKeywords(exceptions), 'gm'), css : 'exceptions'
    },
    {
        regex : new RegExp(this.GetKeywords(types), 'gm'), css : 'types'
    },
    {
        regex : new RegExp(this.GetKeywords(commonlibs), 'gm'), css : 'commonlibs'
    }];
    this.CssClass = 'dp-py';
};
...
dp.sh.Brushes.Perl = function ()
{
    ...
    {
        regex : new RegExp(this.GetKeywords(declarations), 'gm'), css : 'declarations' 
    }];
...
{
    var keywords = 'as assert break case catch class continue def default do else extends finally ' + 'if in implements import instanceof interface new package property return switch ' + 'throw throws try while';
    ...
    {
        regex : new RegExp(this.GetKeywords(modifiers), 'gm'), css : 'modifier'
    },
    {
